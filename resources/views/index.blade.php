            ...
            <div class="flex flex-wrap justify-center gap-3 max-w-2xl w-full">
                @foreach ($agents as $agent)
                    <a href="{{ route('chat.show', $agent) }}"
                        ...
                        @else
                            <div
                                class="w-12 h-12 rounded-lg bg-black/5 dark:bg-white/10 flex items-center justify-center shrink-0">
                                <span
                                    class="text-lg font-semibold text-black/40 dark:text-white/40 select-none">{{ strtoupper(substr($agent->name, 0, 1)) }}</span>
                            </div>
                        @endif
