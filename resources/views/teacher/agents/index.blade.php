                ...
                @foreach ($agents as $agent)
                    <div class="flex items-start gap-3 px-5 py-4">
                        ...
                        <div class="shrink-0 mt-0.5">
                            ...
                            @else
                                <div
                                    ...
                                    <span
                                        class="text-sm font-semibold text-gray-400 dark:text-gray-500">{{ strtoupper(mb_substr($agent->name, 0, 1)) }}</span>
                                </div>
                            @endif
                        </div>
                            ...
                            <div class="flex flex-wrap items-center gap-1.5">
                                ...
                                @elseif ($agent->available_from || $agent->available_until)
                                    <span
                                        ...
                                        {{ substr($agent->available_from ?? '??:??', 0, 5) }} –
                                        {{ substr($agent->available_until ?? '??:??', 0, 5) }}
                                    </span>
                                @else
                                    <span
                        ...
                        <div class="flex shrink-0 items-center gap-2">
                            ...
                            <form method="POST" action="{{ route('teacher.agents.revokeHistory', $agent) }}"
                            ...
                            <form method="POST" action="{{ route('teacher.agents.destroy', $agent) }}"
