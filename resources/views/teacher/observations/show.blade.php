...
@section('title', __('Observation') . ' - ' . config('app.name', 'CurioGPT'))
...
<div class="mx-auto max-w-3xl w-full px-4 py-8">
    ...
    <a href="{{ route('teacher.observations.index') }}"
       class="text-xs text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors">
        ← {{ __('Back to Observations') }}
    </a>

    <div class="mt-3 flex items-center gap-2 flex-wrap">
        <h1 class="text-xl font-semibold text-black dark:text-white">{{ __('Observation') }}</h1>
    ...
    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
        {{ $observation->student_name ?? __('Unknown student') }} · {{
    ...
    @if ($observation->meta)
    <div class="mt-6">
        <h2 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('Metadata') }}</h2>
        <pre
